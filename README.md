...
MedSwarm is a secure and scalable platform for decentralized learning on medical data. This repository contains the **MedSwarmHub** web interface and the local **`medswarm`** companion CLI. Both interfaces share the same Django backend workflow, storage, task queue, and NVFlare runtime so teams can run the same core project, data, network, training, and results workflow through either interface.
...
MedSwarm uses Tailscale for secure peer-to-peer networking.
...
`make install` also installs the local `medswarm` CLI from this same repository. The CLI runs in the same Django environment as MedSwarmHub and is intended for Linux hosts in the current v1 release.
...
`medswarm` is installed from this same repository as a local companion CLI for MedSwarmHub. It uses the same Django environment, database, object storage, Celery workers, and current-project/current-network context as the web UI.
