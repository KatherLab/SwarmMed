...
class BackupConfigurationAdmin(ModelAdmin):
    list_display = ("name", "storage_backend", "is_active", "retention_days", "created_at")
...
class BackupLogAdmin(ModelAdmin):
    list_display = ("started_at", "status", "filename", "file_size_display", "config")
    list_filter = ("status", "config", "started_at")
    readonly_fields = (
        "identifier", "config", "status", "filename", "file_size", 
        "storage_location", "started_at", "finished_at", 
        "error_message", "included_databases", "has_media"
    )
    ...
    @action(description=_("Restore from this Backup"), url_path="trigger-restore")
    def trigger_restore(self, request, queryset):
        ...
        self.message_user(
            request, 
            _("Restore operation triggered in the background. System may be temporarily unavailable."), 
            messages.WARNING
        )
