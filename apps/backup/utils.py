    ...
    if config.storage_backend == StorageBackend.S3:
        if config.s3_access_key_id and config.s3_secret_access_key:
            return S3Boto3Storage(
                ...
                bucket_name=config.s3_bucket or settings.AWS_STORAGE_BUCKET_NAME,
                endpoint_url=config.s3_endpoint_url or settings.AWS_S3_ENDPOINT_URL,
                region_name=config.s3_region_name or settings.AWS_S3_REGION_NAME,
            )
...
def check_postgres_tools():
    ...
    if missing:
        raise Exception(f"Missing PostgreSQL client tools: {', '.join(missing)}")
...
def restore_s3_storage(source_dir):
    ...
    for root, dirs, files in os.walk(s3_src_dir):
...
def perform_backup(log_obj):
    ...
    try:
        ...
        if config.include_s3_storage:
            ...
            exclude_prefix = config.s3_prefix if is_same_s3_destination(config) else None
...
def perform_restore(log_obj):
    config = log_obj.config
    temp_dir = os.path.join(settings.PROJECT_TEMP_DIR, f"restore_{log_obj.identifier}")
    os.makedirs(temp_dir, exist_ok=True)
    ...
    try:
        ...
        if config.storage_backend == StorageBackend.S3:
            ...
            with storage.open(s3_path, "rb") as f_in, open(enc_path, "wb") as f_out:
        ...
        for item in os.listdir(temp_dir):
            if item.startswith("backup_") and os.path.isdir(os.path.join(temp_dir, item)):
