...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            key = base64.urlsafe_b64encode(hashlib.sha256(settings.BACKUP_ENCRYPTION_KEY.encode()).digest())
            fernet = Fernet(key)
            ...
            self.stdout.write(self.style.SUCCESS(f"Encrypted backup uploaded successfully to {s3_path}"))
            ...
            for filename in files:
                if not filename.startswith("db_backup_") or not filename.endswith(".sql.enc"):
                    continue
