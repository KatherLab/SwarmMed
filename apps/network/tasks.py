...
def execute_and_log_in_container(self, container_name: str, command: str, network_id: str, project_id: str, user_id: int):
    ...
    try:
        ...
        LogEntry.objects.create(
            ...
            level='INFO',
            message=f"Executing command in {container_name}: {command}"
        )
        ...
        print(f"ERROR: SwarmNetwork {network_id} not found when executing command.")
    except docker.errors.NotFound:
        print(f"ERROR: Container {container_name} not found for command execution.")
    except Exception as e:
        print(f"CRITICAL: An error occurred in command executor for {container_name}: {e}")
        try:
            ...
            print(f"CRITICAL: Failed to log exception to DB. Main error: {e}, Logging error: {log_e}")