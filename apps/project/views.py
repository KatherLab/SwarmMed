...
from apps.data.utils import get_s3_download_url
...
def project_list(request):
    ...
    ).exclude(status='ARCHIVED').distinct().select_related('author').prefetch_related('members').order_by('-creation_date')
...
def project_edit(request, pk):
    ...
    if request.user != project.author:
        log.access.warning(f"Unauthorized edit attempt for project '{project.title}' by user {request.user.username}")
        return redirect('project:project_list')
...
def project_delete(request, pk):
    ...
    if request.user == project.author:
        ...
        log.access.warning(f"Unauthorized delete attempt for project '{project.title}' by user {request.user.username}")
...
def set_current_project(request, pk):
    ...
    if not (is_author or is_member):
        log.access.warning(f"Unauthorized project access attempt: '{project.title}' by user {request.user.username}")
        return redirect('project:project_list')
    ...
    log.project.debug(f"User {request.user.username} set active project to: '{project.title}'")
...
def project_archive(request, pk):
    ...
    if request.user == project.author:
        if project.status == 'ARCHIVED':
            ...
            UserCurrentProject.objects.filter(user=request.user, project=project).delete()
            messages.success(request, f"Project '{project.title}' archived.")
            log.project.info(f"Project '{project.title}' archived.")
        project.save()
    else:
        log.access.warning(f"Unauthorized archive attempt for project '{project.title}' by user {request.user.username}")
