        ...
        for obj in page.get('Contents', []):
            ...
            if last_modified:
                ...
                job_last_modified[job_id] = max(
                    prev, last_modified) if prev else last_modified
            ...
            try:
                ...
                job = TrainingJob.objects.filter(
                    project=project,
                    flare_job_id__icontains=job_id
                ).first()
...
def start_results_visualization(request, job_id):
    ...
    try:
        ...
        if not py_scripts:
            return JsonResponse({
                'error': (f'No visualization scripts found at {script_prefix}. '
...
def download_all_results(request, project_id):
    ...
    try:
        ...
        base_name = f"{project.title}_{job_filter}_results.zip" if job_filter \
