...
from apps.communication.models import ProjectPost, ProjectBoardAccess, Message
...
class ChatDashboardPerformanceTests(TestCase):
    def setUp(self):
        ...
        self.project1 = Project.objects.create(
            title="Project 1", author=self.user, identifier="11111111-1111-1111-1111-111111111111"
        )
        self.project2 = Project.objects.create(
            title="Project 2", author=self.user, identifier="22222222-2222-2222-2222-222222222222"
        )
        
        # Create posts for Project 1 (User visited in the past)
        # Create a past access log
        past_time = timezone.now() - timedelta(days=1)
        ProjectBoardAccess.objects.create(user=self.user, project=self.project1, last_accessed=past_time)
        
        # Old post (read)
        ProjectPost.objects.create(
            project=self.project1, author=self.user, content="Old post", 
        )
        ...
        ProjectPost.objects.create(project=self.project1, author=self.user, content="New unread post 1")
        ProjectPost.objects.create(project=self.project1, author=self.user, content="New unread post 2")
        
        # Create posts for Project 2 (User NEVER visited)
        ProjectPost.objects.create(project=self.project2, author=self.user, content="Unread post P2")
    ...
    def test_chat_dashboard_unread_counts(self):
        ...
        old_post.timestamp = timezone.now() - timedelta(days=2)
        old_post.save()
