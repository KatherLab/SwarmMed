...
def index(request):
    ...
    for project in user_projects:
        ...
        try:
            ...
            logger.project.debug(
                f"Could not aggregate storage stats for project {project.identifier}: {e}"
            )
            continue
    ...
    try:
        if current_network_obj:
            ...
            job = (
                TrainingJob.objects.filter(network=current_network_obj)
                .order_by("-created_at")
                .first()
            )
            ...
            if job:
                ...
                server_cfg_path = os.path.join(
                    ...
                    str(current_network_obj.identifier),
                    "job",
                    "app_server",
                    "config",
                ...
                for client in ("fl-client-1", "fl-client-2"):
                    ...
                    runs = [
                        d
                        for d in os.listdir(base_client)
