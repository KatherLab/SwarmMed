...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if count > 0:
            self.stdout.write(self.style.SUCCESS(f'Successfully anonymized {count} log entries older than {days} days ({cutoff_date}).'))
        else:
            self.stdout.write(self.style.NOTICE('No log entries found that require anonymization.'))
