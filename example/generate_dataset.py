...
def create_dummy_biomed_data(num_samples=1000, num_files=5, output_dir='biomed_data'):
    """
    ...
    print(f"Generating {num_samples} samples across {num_files} files in '{output_dir}/'...")
    
    for i in range(num_files):
        # Generate random data
        data = {
            'patient_id': range(i*samples_per_file, (i+1)*samples_per_file),
            ...
            'biomarker_A': np.random.exponential(scale=2.0, size=samples_per_file), # skewed feature
            'biomarker_B': np.random.normal(0, 1, samples_per_file) # noise feature
        }
        ...
        logits = (df['age']-50)/20 + (df['bmi']-25)/10 + (df['biomarker_A']-2)/1
        probs = 1 / (1 + np.exp(-logits))
        df['diagnosis'] = (np.random.rand(samples_per_file) < probs).astype(int)
