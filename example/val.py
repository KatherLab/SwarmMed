...
validation.add_check(
    "Validation Started",
    "ok",
    "Beginning biomedical data validation")
...
try:
    ...
    batch_files = [f for f in root_items if f.startswith(
        'batch_') and f.endswith('.csv')]

    if len(batch_files) > 0:
        add_check_with_count(
            "Batch Files",
            "ok",
            f"Found {len(batch_files)} data batch files")
    else:
        add_check_with_count(
            "Batch Files",
            "error",
    ...
    add_check_with_count(
        "Total Volume",
        "ok",
        f"Total dataset size: {total_patients} patients")
